...
class UserService : BaseService<User, UserMapper>() {
    ...
    fun students(flag: GetStudentEnum): List<StudentResult> {
        return when(flag) {
            GetStudentEnum.All -> list()
            GetStudentEnum.HasClass -> TODO()
            GetStudentEnum.NoClass -> baseMapper.getNoClassStudents()
        }.map { StudentResult(it.id!!, it.username) }
    }
     ...
     * @param param 分页参数
     */
    fun page(param: PageParam): BasePage<UserResult> {
